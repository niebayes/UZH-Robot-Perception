...
void print_line() { std::cout << std::string(50, '-') << std::endl; }
...
void test_eigen_libtorch2() {  // RM = Row-major storage (same as LibTorch)
  ...
  // Eigen
  // MatrixXrm<V> = Eigen::Matrix<V, Eigen::Dynamic, Eigen::Dynamic,
  // Eigen::RowMajor>
  MatrixXrm<float> E(3, 3);
  E << 1.010101, 2.020202, 3.030303, 4.040404, 5.050505, 6.060606, 7.070707,
      8.080808, 9.090909;
  std::cout << "EigenMat:\n" << E << std::endl;
  // LibTorch
...
void test_eigen_libtorch3() {  // RM = Row-major storage (same as LibTorch)
  print_line();
  std::cout << "Testing Eigen(RM) to LibTorch (no-copy 3x3 matrix):"
            << std::endl;
  // Eigen
  // MatrixXrm<V> = Eigen::Matrix<V, Eigen::Dynamic, Eigen::Dynamic,
  // Eigen::RowMajor>
...
void test_libtorch_arma2() {
  print_line();
  std::cout << "Testing LibTorch to Armadillo (no-copy 3x3 matrix):"
            << std::endl;
  // LibTorch
  torch::Device device(torch::cuda::is_available() ? torch::kCUDA
                                                   : torch::kCPU);
