...
namespace uzh
{
  ...
  {
    // Convert to arma::Mat
    //! First convert data type to CV_32S for safety.
    cv::Mat matches_cv, query_keypoints_cv, database_keypoints_cv;
    matches.convertTo(matches_cv, CV_32S);
    query_keypoints.convertTo(query_keypoints_cv, CV_32S);
    database_keypoints.convertTo(database_keypoints_cv, CV_32S);
    const arma::urowvec matches_ =
        arma::conv_to<arma::urowvec>::from(uzh::cv2arma<int>(matches_cv).t());
    const arma::umat query_kps =
        arma::conv_to<arma::umat>::from(uzh::cv2arma<int>(query_keypoints_cv).t());
    const arma::umat database_kps = arma::conv_to<arma::umat>::from(
        uzh::cv2arma<int>(database_keypoints_cv).t());
